...
public class CollaborationTypeTest extends JDialog {
    private ProfilePage parent;
    ...
    public CollaborationTypeTest(ProfilePage parent, Test testModel, Management mgr) {
        super(parent, "협업유형 테스트", true);
        this.parent = parent;
        this.test = testModel;
        this.manager = mgr;
        setSize(393, 852);
    ...
    private JPanel createQuestionPage(int index) {
        ...
        nextBtn.addActionListener(e -> {
         System.out.println("▶ 클릭! pageIdx=" + pageIdx + ", selected=" + selectedOptionIndex);
         boolean isLast = (pageIdx == TOTAL_PAGES - 1);

            if (selectedOptionIndex != -1) {
                test.addAnswer(
                    pageIdx + 1,
                 "Q" + (pageIdx + 1) + (char)('A' + selectedOptionIndex)
                );
            }

            if (isLast || selectedOptionIndex != -1) {
    ...
    private JPanel createResultPage() {
        ...
        confirmBtn.setBounds(210, 700, 120, 40);
        confirmBtn.addActionListener(e -> {
            parent.onTestCompleted(test.getUserResultType(), test.getResultImagePath());
            dispose();
        });
    ...
    private void goToNextPage() {
        currentPage++;
        System.out.println("▶ goToNextPage 호출. currentPage=" + currentPage);

        if (currentPage >= TOTAL_PAGES) {
            System.out.println("   → 결과 페이지로 이동");
            cardPanel.add(createResultPage(), "result");
            cardLayout.show(cardPanel, "result");
        } else {
            System.out.println("   → question" + currentPage + " 페이지로 이동");
